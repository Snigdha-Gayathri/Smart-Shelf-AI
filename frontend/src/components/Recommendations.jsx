...
function getBookKey(book) {
  if (!book) return ''
  const hasId = book.id !== undefined && book.id !== null && String(book.id).trim() !== ''
  if (hasId) return `id:${normalizeText(book.id)}`
  return `ta:${normalizeText(book.title)}|${normalizeText(book.author)}`
}

export default function Recommendations({ recommendations = [], onAddToCurrentlyReading, currentlyReading = [], loading = false }) {
  ...
  const currentReadingKeys = useMemo(() => new Set((currentlyReading || []).map(getBookKey)), [currentlyReading])
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
      {recommendations.map((book, idx) => {
        const isInCurrentlyReading = currentReadingKeys.has(getBookKey(book))
        return (
        <CategoryStyledBookCard
          ...
          forceDodgerOutline
          showReason={false}
          cornerRibbonText={isInCurrentlyReading ? 'Already in Currently Reading' : ''}
        >
